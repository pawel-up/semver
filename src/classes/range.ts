...
import { safeRe as re, t, comparatorTrimReplace, tildeTrimReplace, caretTrimReplace } from '../internal/re.js'
...
export default class Range {
  ...
  parseRange(range: string): Comparator[] {
    ...
    const memoOpts =
      Number(this.options.includePrerelease && FLAG_INCLUDE_PRERELEASE) | Number(this.options.loose && FLAG_LOOSE)
  ...
  test(version: string | SemVer): boolean {
    ...
    for (const item of this.set) {
      if (testSet(item, version, this.options)) {
        return true
      }
    }
    return false
...
const isSatisfiable = (comparators: Comparator[], options?: Options | boolean): boolean => {
  let result = true
  if (!comparators.length) {
    return result
  }
  const remainingComparators = [...comparators]
  let testComparator = remainingComparators.pop() as Comparator
  ...
  while (result && remainingComparators.length) {
    ...
    testComparator = remainingComparators.pop() as Comparator
  }
...
const parseComparator = (comp: string, options?: Options) => {
...
const isX = (id: string): boolean => !id || id.toLowerCase() === 'x' || id === '*'
...
// ~0.0.1 --> >=0.0.1 <0.1.0-0
const replaceTildes = (comp: string, options?: Options) => {
  return comp
    .trim()
    .split(/\s+/)
...
const replaceTilde = (comp: string, options: Options = {}) => {
...
// ^0.0.1 --> >=0.0.1 <0.0.2-0
// ^0.1.0 --> >=0.1.0 <0.2.0-0
const replaceCarets = (comp: string, options?: Options) => {
  return comp
    .trim()
    .split(/\s+/)
...
const replaceCaret = (comp: string, options: Options = {}) => {
...
const replaceXRanges = (comp: string, options?: Options) => {
...
const replaceXRange = (comp: string, options: Options = {}) => {
  comp = comp.trim()
...
const replaceStars = (comp: string, options?: Options) => {
  debug('replaceStars', comp, options)
  // Looseness is ignored here.  star is always as loose as it gets!
  return comp.trim().replace(re[t.STAR], '')
}

const replaceGTE0 = (comp: string, options: Options = {}) => {
...
const testSet = (set: Comparator[], version: SemVer, options: Options = {}) => {
  ...
  if (version.prerelease.length && !options.includePrerelease) {
    ...
    for (const item of set) {
      debug(item.semver)
      if (!Comparator.isSemver(item.semver)) {
        continue
      }
      if (item.semver.prerelease.length > 0) {
        const allowed = item.semver
